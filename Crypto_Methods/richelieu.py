...
def show_err(key: str, err=None):
    ...
    if err:
        error.setText('НЕВЕРНЫЙ КЛЮЧ!')
        error.setInformativeText('Проверьте ключ на правильность')
        error.exec_()
        return 1

    key_check_1 = key.replace(' ', '')
    key_check_1 = key_check_1.replace(',', '')
    if not key_check_1.isdigit():
        error.setText('НЕВЕРНЫЙ СИМВОЛ!')
        error.setInformativeText('Ключ должен содержать только цифры, запятые (разделители в блоке) и пробелы '
                                 '(разделители блоков)')
        error.exec_()
        return 1


    key_check_2 = key.split()
    for block in key_check_2:
        block = block.split(',')
        for ch in block:
            if ch == '':
                block.pop(block.index(ch))
                continue
            if len(block) < int(ch) or '0' in block:
                error.setText('НЕВЕРНЫЙ КЛЮЧ!')
                error.setInformativeText('Проверьте правильность ключа (невнерый индекс элементов в блоках)')
                error.exec_()
                return 1
    ...
    for block in key_check_2:
        ...
        for i in block:
            if block.count(i) > 1:
                error.setText('НЕВЕРНЫЙ КЛЮЧ!')
                error.setInformativeText('Проверьте правильность ключа (повторения в блоке недопустимы)')
                error.exec_()
                return 1
...
def richelieu(text: str, key: str):
    encrypted_text = ''
    tmp = ''
    key = key.split()
    for block in key:
...
"")
    ...
    def add_functions(self):
        self.encrypt_btn.clicked.connect(lambda: self.encrypt(self.text_edit.toPlainText(), self.lineEdit.text()))
        self.decrypt_btn.clicked.connect(lambda: self.decrypt(self.text_edit_encrypted.toPlainText(),
                                                              self.lineEdit.text()))

    def encrypt(self, text, key):
        if length(key) > len(text) or key[0] == ',' or key[len(key) - 1] == ',' or ',,' in key:
            show_err(key, True)
            return 0
        if show_err(key) == 1:
            return 0
        encrypted_text = richelieu(text, key)
    ...
    def decrypt(self, text, key):
        ...
        for block in key:
            ...
            if '' in block:
                show_err(key, True)
            for i in range(len(block)):
                tmp += str(block.index(str(i + 1)) + 1) + ','
            tmp = tmp[:-1]
            decrypt_key += tmp + ' '
            tmp = ''

        decrypted_text = richelieu(text, decrypt_key)
        self.textBrowser_decrypted.setPlainText(decrypted_text)

    def retranslateUi(self, MainWindow):
        _translate = QtCore.QCoreApplication.translate
        MainWindow.setWindowTitle(_translate("MainWindow", "Ришелье"))
