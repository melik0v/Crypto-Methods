...
def show_err():
    ...
    error.setInformativeText('Поддерживается только кириллица верхнего регистра и пробелы, '
                ...
                for item in tmp:
                    ...
                    for i in range(len(ru_encrypt_matrix)):
                        for j in range(len(ru_encrypt_matrix[i])):
                            if ru_encrypt_matrix[i][j].find(item) != -1 and ch != '/':
                                found = True
                    if not found:
                        show_err()
                        return 0
                encrypted_text += encrypt_function(tmp)
                tmp = ''
                encrypted_text += ch
    ...
    def decrypt(self, text):
        ...
        for ch in text:
            ...
            if ch != ' ':
                ...
                decrypted_text += decrypt_function(tmp)
                tmp = ''
                decrypted_text += ch
