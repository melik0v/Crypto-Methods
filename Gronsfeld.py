...
def show_err(code: int):
    ...
    if code == 2:
        error.setText('НЕВЕРНЫЙ СИМВОЛ!')
        error.setInformativeText('В ключе допустимы только цифры!')
        error.exec_()
...
"")
    ...
    def retranslateUi(self, MainWindow):
        ...
        MainWindow.setWindowTitle(_translate("MainWindow", "Gronsfeld"))
    ...
    def encrypt(self, text, key):
        ...
        for i, j in zip(text, text_key):
            ...
            if i in en_alph_lower:
                encrypted_text += np.roll(en_alph_lower, -int(j))[en_alph_lower.index(i)]
                continue

            elif i in en_alph_upper:
                encrypted_text += np.roll(en_alph_upper, -int(j))[en_alph_upper.index(i)]
                continue

            elif i in ru_alph_lower:
                encrypted_text += np.roll(ru_alph_lower, -int(j))[ru_alph_lower.index(i)]
                continue

            elif i in ru_alph_upper:
                encrypted_text += np.roll(ru_alph_upper, -int(j))[ru_alph_upper.index(i)]
                continue
            else:
                encrypted_text += i
    ...
    def decrypt(self, text, key):
        """
                ...
                :param key: Key word
                :param table: Table of alphabets
        """
        # key under the text
        text_key = ''
        if not key.isdigit():
        ...
        for i, j in zip(text, text_key):
            # looking for which alphabet the text symbol belongs to
            if i in en_alph_lower:
                rolled_alph = tolist(np.roll(en_alph_lower, -int(j)))
                decrypted_text += en_alph_lower[rolled_alph.index(i)]
                continue

            elif i in en_alph_upper:
                rolled_alph = tolist(np.roll(en_alph_upper, -int(j)))
                decrypted_text += en_alph_upper[rolled_alph.index(i)]
                continue
            
            elif i in ru_alph_lower:
                rolled_alph = tolist(np.roll(ru_alph_lower, -int(j)))
                decrypted_text += ru_alph_lower[rolled_alph.index(i)]
                continue
            
            elif i in ru_alph_upper:
                rolled_alph = tolist(np.roll(ru_alph_upper, -int(j)))
