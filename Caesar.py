        ...
        if not found:
            for ch in digit_alph:
                if i == ch:
                    ...
                    encrypted_text += digit_alph[(pos + key) % len(digit_alph)]
                    found = True
        ...
        if not found:
            for ch in en_alph_lower:
                if i == ch:
                    ...
                    encrypted_text += en_alph_lower[(pos + key) % len(en_alph_lower)]
                    found = True
                    break
        ...
        if not found:
            for ch in en_alph_upper:
                if i == ch:
                    ...
                    encrypted_text += en_alph_upper[(pos + key) % len(en_alph_upper)]
                    found = True
                    break
        ...
        if not found:
            for ch in ru_alph_lower:
                if i == ch:
                    ...
                    encrypted_text += ru_alph_lower[(pos + key) % len(ru_alph_lower)]
                    found = True
                    break
        ...
        if not found:
            for ch in ru_alph_upper:
                if i == ch:
                    ...
                    encrypted_text += ru_alph_upper[(pos + key) % len(ru_alph_upper)]
                    found = True
                    break
...
"")
        ...
        self.key_spinBox.setMinimum(-99)
        self.key_spinBox.setMaximum(99)
    ...
    def add_functions(self):
        self.encrypt_btn.clicked.connect(lambda: self.encrypt(self.text_edit.toPlainText(), int(self.key_spinBox.text())))
        self.decrypt_btn.clicked.connect(lambda: self.decrypt(self.text_edit_encrypted.toPlainText(),
                                                              int(self.key_spinBox.text())))

    def encrypt(self, key, text):
        encrypted_text = caesar(key, text)
        self.textBrowser_encrypted.setPlainText(encrypted_text)

    def decrypt(self, text, key):
        key = key * (-1)
        decrypted_text = caesar(text, key)
