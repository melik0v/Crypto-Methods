...
# Form implementation generated from reading ui file 'Alberti.ui'
#
...
def show_err(code: int):
    ...
    if code == 1:
        ...
        error.setInformativeText('Поддерживается только латиница нижнего регистра, '
                                 'проверьте текст на соответствие требованиям')
        error.exec_()
    if code == 3:
        error.setText('НЕВЕРНЫЙ СИМВОЛ!')
        error.setInformativeText('Поддерживается только латиница верхнего регистра, '
                                 'проверьте текст на соответствие требованиям')
        error.exec_()
...
"font-weight: 700;")
        ...
        self.encrypt_btn.setGeometry(QtCore.QRect(390, 220, 130, 30))
        font = QtGui.QFont()
        font.setFamily("Century Gothic")
...
"")
        ...
        self.label_2.setGeometry(QtCore.QRect(20, 240, 131, 16))
        font = QtGui.QFont()
        font.setFamily("Century Gothic")
        font.setPointSize(14)
        font.setBold(True)
        ...
        self.initial_shift_spinBox = QtWidgets.QSpinBox(self.centralwidget)
        self.initial_shift_spinBox.setGeometry(QtCore.QRect(300, 200, 61, 31))
        font = QtGui.QFont()
        font.setFamily("Century Gothic")
        font.setPointSize(14)
        font.setBold(True)
        font.setWeight(75)
        self.initial_shift_spinBox.setFont(font)
        self.initial_shift_spinBox.setStyleSheet("color: white;")
        self.initial_shift_spinBox.setObjectName("initial_shift_spinBox")
        self.initial_shift_spinBox.setMinimum(-99)
        self.shift_spinBox = QtWidgets.QSpinBox(self.centralwidget)
        self.shift_spinBox.setMinimum(-99)
        self.shift_spinBox.setGeometry(QtCore.QRect(160, 230, 61, 31))
        font = QtGui.QFont()
        font.setFamily("Century Gothic")
        font.setPointSize(14)
        font.setBold(True)
        font.setWeight(75)
        self.shift_spinBox.setFont(font)
        self.shift_spinBox.setStyleSheet("color: white;")
    ...
    def retranslateUi(self, MainWindow):
        ...
        MainWindow.setWindowTitle(_translate("MainWindow", "Альберти"))
        ...
        self.label.setText(_translate("MainWindow", "НАЧАЛЬНОЕ СМЕЩЕНИЕ"))
        self.label_2.setText(_translate("MainWindow", "СМЕЩЕНИЕ"))

    def add_functions(self):
        # rand_alph = random.sample(en_alph_upper, len(en_alph_upper))
        rand_alph = en_alph_upper
        self.encrypt_btn.clicked.connect(lambda: self.encrypt(self.text_edit.toPlainText(),
                                                              self.initial_shift_spinBox.value(),
                                                              self.shift_spinBox.value(), rand_alph))

        self.decrypt_btn.clicked.connect(lambda: self.decrypt(self.text_edit_encrypted.toPlainText(),
                                                              self.initial_shift_spinBox.value(),
                                                              self.shift_spinBox.value(), rand_alph))

    def encrypt(self, text, initial_shift, shift, movable_alphabet):
        """
        Encryption by the alberti algorithm
        :param text: Text to decrypt
        :param initial_shift: Initial shift of disk
        :param shift: Shift per iteration
        :param movable_alphabet: Mixed movable alphabet
        """
        rand_alph = np.roll(movable_alphabet, -initial_shift)

        encrypted_text = ''

        for i in text:
            if i not in en_alph_lower:
                encrypted_text += i
                continue
            encrypted_text += rand_alph[en_alph_lower.index(i)]
            rand_alph = np.roll(rand_alph, -shift)

        self.textBrowser_encrypted.setPlainText(encrypted_text)

    def decrypt(self, text, initial_shift, shift, movable_alphabet):
        """
        ...
        """
        rand_alph = np.roll(movable_alphabet, -initial_shift)
        rand_alph = tolist(rand_alph)
        decrypted_text = ''
        for i in text:
            if i not in en_alph_upper:
                decrypted_text += i
                continue
            decrypted_text += en_alph_lower[rand_alph.index(i)]
            rand_alph = np.roll(rand_alph, -shift)
            rand_alph = tolist(rand_alph)
